▸ 中文区广告过滤规则整合  
Combined Mobile Ad Filters
___
📥  

[Jsdelivr](https://cdn.jsdelivr.net/gh/Lky777/MWCP@main/rules/MobiListChina.txt)  
```html
https://cdn.jsdelivr.net/gh/Lky777/MWCP@main/rules/MobiListChina.txt
```

[GitHub Link](https://raw.githubusercontent.com/Lky777/MWCP/refs/heads/main/rules/MobiListChina.txt)  
```html
https://raw.githubusercontent.com/Lky777/MWCP/refs/heads/main/rules/MobiListChina.txt
```

___
**Credits**  
* [easylist](https://github.com/easylist)  
* [badmojr](https://github.com/badmojr)   
* [cjx82630](https://github.com/cjx82630)  
* [xinggsf](https://github.com/xinggsf)    

___
声明  
本项目**仅限用于**：广告过滤技术学习，隐私保护开发研究 。**禁止用于**任何商业、营利或侵权活动。  

本项目规则集基于第三方资源整合，维护者**不保证**： 
* 100%广告拦截率  
* 完全不影响网页功能  
* 绝对安全性（规则可能被篡改风险）  
